...
//$_rbox_$_modify_$_lijiehong:_begin_$ add by lijiehong to support configure more keys panel
    ...
    private CheckBoxPreference mShowMoreKeysPanelPreference;
    private PreferenceScreen mLongPressTimeoutSettingsPref;
    private TextView mLongPressTimeoutSettingsTextView;
...
//$_rbox_$_modify_$_lijiehong:_begin_$ add by lijiehong to support configure more keys panel
        mShowMoreKeysPanelPreference = (CheckBoxPreference) findPreference(PREF_SHOW_MORE_KEYS_PANEL);
        mLongPressTimeoutSettingsPref =
                (PreferenceScreen) findPreference(PREF_LONG_PRESS_TIMEOUT);
        if (mLongPressTimeoutSettingsPref != null) {
            mLongPressTimeoutSettingsPref.setOnPreferenceClickListener(
                    new OnPreferenceClickListener() {
                        @Override
                        public boolean onPreferenceClick(Preference arg0) {
                            showLongPressTimeoutSettingDialog();
                            return true;
                        }
                    });
            updateLongPressTimeoutSummary(prefs, res);
        }
//$_rbox_$_modify_$_lijiehong:_end_$ add by lijiehong to support configure more keys panel
...
//$_rbox_$_modify_$_lijiehong:_begin_$ add by lijiehong to support configure more keys panel
    private void updateLongPressTimeoutSummary(SharedPreferences sp, Resources res) {
        if (mLongPressTimeoutSettingsPref != null) {
            mLongPressTimeoutSettingsPref.setSummary(
                    Utils.getCurrentLongPressTimeout(sp)
                            + res.getString(R.string.settings_ms));
        }
    }

    private void showLongPressTimeoutSettingDialog() {
        final SharedPreferences sp = getPreferenceManager().getSharedPreferences();
        final Context context = getActivity();
        final Resources res = context.getResources();
        final AlertDialog.Builder builder = new AlertDialog.Builder(context);
        builder.setTitle(R.string.prefs_long_press_timeout_settings);
        builder.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int whichButton) {
                final int ms = Integer.valueOf(
                        mLongPressTimeoutSettingsTextView.getText().toString());
                sp.edit().putInt(Settings.PREF_LONG_PRESS_TIMEOUT, ms).apply();
                updateLongPressTimeoutSummary(sp, res);
            }
        });
        builder.setNegativeButton(android.R.string.cancel,  new DialogInterface.OnClickListener() {
            @Override
        ...
        final View v = LayoutInflater.from(context).inflate(
                R.layout.vibration_settings_dialog, null);
        final int currentMs = Utils.getCurrentLongPressTimeout(
                getPreferenceManager().getSharedPreferences());
        mLongPressTimeoutSettingsTextView = (TextView)v.findViewById(R.id.vibration_value);
        final SeekBar sb = (SeekBar)v.findViewById(R.id.vibration_settings);
        sb.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar arg0, int arg1, boolean arg2) {
                final int tempMs = arg1;
                mLongPressTimeoutSettingsTextView.setText(String.valueOf(tempMs*20));
            }
        ...
        sb.setProgress(currentMs/20);
        mLongPressTimeoutSettingsTextView.setText(String.valueOf(currentMs));
        builder.setView(v);
        builder.create().show();
    }
//$_rbox_$_modify_$_lijiehong:_end_$ add by lijiehong to support configure more keys panel
