...
//$_rbox_$_modify_$_lijiehong: begin add by lijiehong to support remote-ctrl
...
//$_rbox_$_modify_$_lijiehong: end add by lijiehong
...
//$_rbox_$_modify_$_lijiehong: begin changed by lijiehong to support configure moreKeysPanel
        @Override
        public void startLongPressTimer(final PointerTracker tracker, int delay) {
            cancelLongPressTimer();
            if (tracker == null) {
                return;
            }
            final Key key = tracker.getKey();
            //final int delay;//modify by lijiehong
            switch (key.mCode) {
            case Keyboard.CODE_SHIFT:
                delay = mLongPressShiftKeyTimeout;
                break;
            default:
                if (KeyboardSwitcher.getInstance().isInMomentarySwitchState()) {
                    ...
                    // mode key.
                    delay = /*mLongPressKeyTimeout*/delay * 3;//modify by lijiehong;
                } else {
                    //delay = mLongPressKeyTimeout;//modify by lijiehong
                }
                break;
            }
            if (delay > 0) {
...
//$_rbox_$_modify_$_lijiehong: end changed by lijiehong to support configure moreKeysPanel
...
//$_rbox_$_modify_$_lijiehong:_begin_$ add by lijiehong to support configure more keys panel
    private SharedPreferences mPrefs;//add by lijiehong

    public SharedPreferences getPreferences() {//add by lijiehong
        return mPrefs;
    }
    public MainKeyboardView(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        mPrefs = PreferenceManager.getDefaultSharedPreferences(context);//add by lijiehong
...
//$_rbox_$_modify_$_lijiehong:_end_$ add by lijiehong to support configure more keys panel
...
//$_rbox_$_modify_$_lijiehong: begin add by lijiehong for initializing focus of mini-keyboard
...
//$_rbox_$_modify_$_lijiehong: added by lijiehong for internet tv Key
    ...
    public void SendKeyEvent2Touch(int keyCode){
        ...
        if(mMoreKeysPanel!=null){
            ...
            if(action ==MotionEvent.ACTION_UP ){//refocus the key after clicked
                SystemClock.sleep(100);
                m_iIndexCurKey = FindKeyIndex(keys,x,y);//keyboard layout may have been changed, recaculate the key index
                //if(keys.get(m_iIndexCurKey).mCode>0){
                action = MotionEvent.ACTION_DOWN;
               // Log.i(TAG, "######## m_iIndexCurKey="+m_iIndexCurKey+"---action ="+action+"---code ="+keys.get(m_iIndexCurKey).mCode);
                me_temp = MotionEvent.obtain(SystemClock.uptimeMillis(),
                            SystemClock.uptimeMillis(),action,x, y, 0);
                onTouchEvent(me_temp);
                //}
...
//$_rbox_$_modify_$_lijiehong: end add by lijiehong
    ...
    public boolean processMotionEvent(final MotionEvent me) {
        ...
        final int x, y;
//$_rbox_$_modify_$_lijiehong: begin modify by lijiehong for supporting remote controller
        if (mMoreKeysPanel != null && id == mMoreKeysPanelPointerTrackerId &&!mIsRemoteCtrl) {
//$_rbox_$_modify_$ end use origin x and y while using remoteController
...
//$_rbox_$_modify_$_lijiehong: begin modify by lijiehong for supporting remote controller
                if (mMoreKeysPanel != null
                        && tracker.mPointerId == mMoreKeysPanelPointerTrackerId && !mIsRemoteCtrl) {
//$_rbox_$_modify_$ end use origin x and y while using remoteController
