...
//$_rbox_$_modify_$_lijiehong:_begin_$ add by lijiehong for releasing old key and change some key actions
        ...
        if(newKey!=null && mCurrentKey!=null){
            if(!mCurrentKey.equals(newKey)){
                setReleasedKeyGraphics(mCurrentKey);
                if(newKey.mCode==Keyboard.CODE_DELETE || newKey.mCode==Keyboard.CODE_SHIFT
                    || newKey.mCode==Keyboard.CODE_SWITCH_ALPHA_SYMBOL){
...
//$_rbox_$_modify_$_lijiehong:_begin_$ add by lijiehong , ignore repeat delete key, do not process shift key and symbol switch key on key down
            ...
            if(key.mCode==Keyboard.CODE_DELETE || key.mCode==Keyboard.CODE_SHIFT
                || key.mCode==Keyboard.CODE_SWITCH_ALPHA_SYMBOL){
...
//$_rbox_$_modify_$_lijiehong: begin change by lijiehong , do process shift key and symbol switch key on keyUp
        ...
        if(mCurrentKey!=null){
            setReleasedKeyGraphics(mCurrentKey);
            if(mCurrentKey.mCode==Keyboard.CODE_DELETE || mCurrentKey.mCode==Keyboard.CODE_SHIFT
                || mCurrentKey.mCode==Keyboard.CODE_SWITCH_ALPHA_SYMBOL){
                callListenerOnPressAndCheckKeyboardLayoutChange(mCurrentKey);
...
//$_rbox_$_modify_$_lijiehong:_begin_$ add by lijiehong to support configure more keys panel
    public int getLongPressKeyTimeout(){//add by lijiehong
        if(mDrawingProxy!=null)
            return Utils.getCurrentLongPressTimeout(((MainKeyboardView)mDrawingProxy).getPreferences());
        return 1500;//default use 1500ms
    }

    private void startLongPressTimer(final Key key) {
        if (key != null && key.isLongPressEnabled() && !sInGesture) {
            int delay = getLongPressKeyTimeout();//add by lijiehong
            mTimerProxy.startLongPressTimer(this,delay);//modify by lijiehong
        }
    }
//$_rbox_$_modify_$_lijiehong:_end_$ modify by lijiehong
