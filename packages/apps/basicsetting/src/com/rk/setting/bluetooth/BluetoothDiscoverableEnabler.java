...
final class BluetoothDiscoverableEnabler
{
	...
	private final String BLUETOOTH_SHAREPREFRENCE = "bluetooth_preferences";
    ...
    BluetoothDiscoverableEnabler(Context context,   Handler handler,        LocalBluetoothAdapter adapter)
	{
        mContext = context;
				mHandler = handler;
        mLocalAdapter = adapter;
        mSharedPreferences = mContext.getSharedPreferences(BLUETOOTH_SHAREPREFRENCE,Context.MODE_WORLD_WRITEABLE|Context.MODE_WORLD_READABLE);
    }
    ...
    public void pause() {
        if (mLocalAdapter == null) {
  		...
  		mHandler.removeCallbacks(mUpdateCountdownSummaryRunnable);
    ...
    private void updateTimerDisplay(int timeout) {
        ...
        if (getDiscoverableTimeout() == DISCOVERABLE_TIMEOUT_NEVER) {
			summary = mContext.getResources().getString(R.string.bluetooth_is_discoverable_always);
	//		((RKSettings)mContext).updateSettingItem(R.string.bluetooth_visibility,null,
	//						summary,null);
        } else {
            String textTimeout = formatTimeRemaining(timeout);
			summary = mContext.getString(R.string.bluetooth_is_discoverable,textTimeout);
	//		((RKSettings)mContext).updateSettingItem(R.string.bluetooth_visibility,null,
	//						summary,null);
        }
// 		mHandler.sendEmptyMessage(SettingMacroDefine.upDateListView);
    ...
    private void setSummaryNotDiscoverable() {
        ...
        if (mNumberOfPairedDevices != 0) {
			summary = mContext.getResources().getString(R.string.bluetooth_only_visible_to_paired_devices);
	//		((RKSettings)mContext).updateSettingItem(R.string.bluetooth_visibility,null,
	//						summary,null);
        } else {
        	summary = mContext.getResources().getString(R.string.bluetooth_not_visible_to_other_devices);
	//		((RKSettings)mContext).updateSettingItem(R.string.bluetooth_visibility,null,
	//						summary,null);
        }
//		mHandler.sendEmptyMessage(SettingMacroDefine.upDateListView);
    ...
    private void updateCountdownSummary() {
        ...
        synchronized (this) {
			mHandler.removeCallbacks(mUpdateCountdownSummaryRunnable);
            mHandler.postDelayed(mUpdateCountdownSummaryRunnable, 1000);
        }
    }
}
