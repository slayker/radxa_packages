package com.rk.setting.wifi.android_4_2;
...
//			Log.d("wifiSetting","access.getLevel() = "+access.getLevel());
			...
			{
			//	onClickCallback();
                mSelected.generateOpenNetworkConfig();
                mWifiManager.connect(mSelected.getConfig(), mConnectListener);
			}
			else
			{
				mParentInput = (ViewGroup)findViewById(R.id.container_input);
	...
	private void createSwitch()
	{
		...
		mSwitch.setOnFocusChangeListener(new View.OnFocusChangeListener()
		{
			...
			{
				Log.d("WifiSetting","mSwitch focus changed. hasFocus = "+(hasFocus?"true":"false"));
				if(hasFocus)
				{
					boolean on = mWifiEnabler.getWifiStatus();
	...
	public void onResume()
	{
		...
		if(mWifiEnabler != null)
		{
			...
			if((on) && (mAccessPoints.size() > 0) && mGallery != null)
			{
				...
				mAdapter.setFocus(mGallery.hasFocus());
				mFocus = mParentDevice.findFocus();
					...
					//		if(!mGallery.isMoving())
							{
								...
								Log.d("WifiSetting","updateAccessPoints(), mGallery.hasFocus() = "+mGallery.hasFocus());
								mGallery.setSelection(mGallerySelect);
								mGallery.setStayPosition(true);
								boolean focus = ((mFocus != null) && (mSwitch != mFocus));
								mAdapter.setFocus(focus);//(mGallery == mFocus);
...
/*
	...
	public boolean dispatchKeyEvent(KeyEvent event)
	{
		...
		if(mParentDevice.getVisibility() == View.VISIBLE)
		{
			View view = ((ViewGroup)mParentDevice).getFocusedChild();
			if(view != null)
			{
				if((event.getAction() == KeyEvent.ACTION_DOWN))
				{
					...
					{
						mFocus = view.focusSearch(View.FOCUS_DOWN);
					}
					else if(keyCode == KeyEvent.KEYCODE_DPAD_UP)// up
					{
						mFocus = view.focusSearch(View.FOCUS_UP);
					}
					else if(keyCode == KeyEvent.KEYCODE_DPAD_LEFT)// left
					{
						mFocus = view.focusSearch(View.FOCUS_LEFT);
					}
					else if(keyCode == KeyEvent.KEYCODE_DPAD_RIGHT)// right
					{
						mFocus = view.focusSearch(View.FOCUS_RIGHT);
					}
				}

		/*		if(mFocus != null)
				{
					Log.d("WifiSetting","Focus View id = "+view.getId()+", next Focus id = "+mFocus.getId()+",mSwitch id = "+mSwitch.getId()+",MyGallery id = "+mGallery.getId());
				}
				else
				{
					Log.d("WifiSetting","mFocus = null");
				}*/
			}
