...
public final class Launcher extends Activity
    ...
    private static final int WALLPAPER_SCREENS_SPAN = 5;
 ...
 //private static final int MENU_WALLPAPER_SETTINGS = MENU_MANAGE_APPS + 1;
    private static final int MENU_SEARCH = MENU_MANAGE_APPS + 1;
...
//    private SharedPreferences mPreferences;
    ...
    protected void onCreate(Bundle savedInstanceState) {
	 ...
	 getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_FULLSCREEN); //for pad
...
//	 mPreferences = this.getSharedPreferences("MyFolder",Context.MODE_WORLD_WRITEABLE|Context.MODE_WORLD_READABLE);
    }
	...
	{		
		...
		ScreenInfo.mScreenWidth = displayMetrics.widthPixels;	        
		ScreenInfo.mScreenHeight = displayMetrics.heightPixels;		 
		ScreenInfo.mDensityDpi = displayMetrics.densityDpi;		 
		ScreenInfo.mDpiRatio = ((float)ScreenInfo.mDefaultDpi)/(float)displayMetrics.densityDpi;
	}
    ...
    private void setWallpaperDimension() {
        ...
        wpm.suggestDesiredDimensions(display.getWidth() * WALLPAPER_SCREENS_SPAN/4, display.getHeight());
    }
    ...
    */
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
	 ...
	 Log.d(TAG, " onKeyDown keyCode = " + keyCode + " ;   handled = " + handled);
	 if(handled==false){
    ...
    private void setupViews() {
	 ...
	 //chengmc@rock-chips.com:mPreviousView and mNextView is Hiden
	 View apps_cluster = findViewById(R.id.all_apps_button_cluster);
	 apps_cluster.setVisibility(View.GONE);
    ...
    public void launchHotSeat(View v) {
        ...
        if (index >= 0 && index < mHotseats.length && mHotseats[index] != null) {
            ...
            startActivitySafely(v, mHotseats[index], "hotseat");
        }
    }
    ...
    View createBookmark(ShortcutInfo info){
	 TextView favorite = (TextView) mInflater.inflate(R.layout.application,
               	 (ViewGroup) mWorkspace.getChildAt(mWorkspace.getCurrentScreen()), false);
	 
	 Bitmap icon  = null;
	 String text = null;

	favorite.setBackgroundResource(R.drawable.shortcut);
	favorite.setGravity(Gravity.CENTER);
	Pattern p = Pattern.compile("(?<=http://|\\.)[^.]*?\\.(com|cn|net|org|biz|info|cc|tv)",Pattern.CASE_INSENSITIVE);
	Matcher matcher = p.matcher(info.intent.toURI().toString());
	matcher.find();
	text = matcher.group().replace(".com", "");

    int width = ScreenInfo.mScreenWidth/5;
    int height = ScreenInfo.mScreenHeight/3;
	favorite.setTextSize(height/6);
    ...
    View createShortcut(int layoutResId, ViewGroup parent, ShortcutInfo info) {	
	 ...
	 icon = getScaleImage(icon);
	 FastBitmapDrawable xIcon = new FastBitmapDrawable(icon);
	 favorite.setCompoundDrawablesWithIntrinsicBounds(null, new FastBitmapDrawable(icon), null, null);
	 favorite.setBackgroundResource(R.drawable.background_grey);
	 text = info.title.toString();

     int width = ScreenInfo.mScreenWidth/NUMBER_CELLS_X;
     int height = ScreenInfo.mScreenHeight/3;
    ...
    private Bitmap getScaleImage(Bitmap image){
	 if(null==image){return null;}
        int width = ScreenInfo.mScreenWidth/NUMBER_CELLS_X;
        float scale = (width/2.0f)/(image.getWidth())*1.2f;
    ...
    View testView(int layoutResId, ViewGroup parent, ShortcutInfo info){
    ...
    private Bitmap bitMapScale(Bitmap map, float scalex, float scaley){
		if(map == null)  return null;
		
		int width = (int)((float)map.getWidth()*scalex);
		int height = (int)((float)map.getHeight()*scaley);
    ...
    private void completeAddShortcut(Intent data, CellLayout.CellInfo cellInfo) {
        ...
        if (!mRestoring) {
            final View view = createBookmark(info);
    ...
    private void manageApps() {
	 ...
	 //overridePendingTransition(android.R.anim.zoom_enter, android.R.anim.zoom_exit);
...
/*
	 ...
	 {
		// when the folder is deleted, then set the value of folder id in xml file to -1
		long id = read(mPreferences,KEY[i]);
		if(id == folder.id)
		{
			store(mPreferences,KEY[i],-1L);
			break;
		}
	 }
	 loadMyFolder = false;
...
//        }
        ...
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            switch (event.getKeyCode()) {
                ...
                case KeyEvent.KEYCODE_VOLUME_DOWN:
                    if (SystemProperties.getInt("debug.launcherNew.dumpstate", 0) != 0) {
                        dumpState();
                        return true;
                    }
                    break;
    ...
    void startActivitySafely(View v, Intent intent, Object tag) {
    ...
    boolean startActivity(View v, Intent intent, Object tag) {
        ...
        try {
            ...
            if (useLaunchAnimation) {
                Log.d(TAG, "Common   Animation, defalut");
    ...
    private void openFolder(FolderInfo folderInfo) {
        ...
        if (folderInfo instanceof UserFolderInfo) {
            ...
            openFolder = com.android.launcherNew.LiveFolder.fromXml(this, folderInfo);
        } else {
            return;
        }
    ...
    public boolean onLongClick(View v) {
        ...
        if (cellInfo == null || (cellInfo.cellY==1)) {
            return true;
        }
		Log.d(TAG , "CellInfo.tag = " + v.getTag().toString());
		View xx = cellInfo.cell;
		if(null != xx){
			Log.d(TAG, "CellInfo.width =" + xx.getWidth() + "; CellInfo.height=" + xx.getHeight());
		}
    ...
    private void selectShortcut(){
		...
		intent.setClassName("com.android.browser", "com.android.browser.ShortcutActivity");
    ...
    public void bindItems(ArrayList<ItemInfo> shortcuts, int start, int end) {
        ...
        for (int i=start; i<end; i++) {
            ...
            switch (item.itemType) {
		     ...
		     final View app = createShortcut((ShortcutInfo)item);
                   workspace.addInScreen(app, item.screen, item.cellX, item.cellY, 1, 1, false);
		     break;
                case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
		     final View shortcut = createBookmark((ShortcutInfo)item);
		     /*selfdefined bookmark*/
		     if(!((ShortcutInfo)item).title.toString().startsWith("URL") ){
			    workspace.addInScreen(shortcut, item.screen, item.cellX, item.cellY, 1, 1, true);
			    break;
		     }
		     /*defalut bookmark*/
		     if(Locale.getDefault().getLanguage().startsWith("zh") ){
			    if(((ShortcutInfo)item).title.toString().startsWith("URL-zh-rcn") ){
...
//	 addApplicationToFolder(apps);
    ...
    public void dumpState() {
        Log.d(TAG, "BEGIN launcherNew dump state for launcher " + this);
        ...
        Log.d(TAG, "END launcherNew dump state");
    }
/*
      // used to as a key to store Folder in file. The order and position must be as the same as R.array.myfolder defined in Strings.xml
	private String KEY[] = {"Game","Tool","Video"};
	private boolean mFodlerHasBuild[] = {true,true,true};
	private boolean loadMyFolder = true;
	public void addMyFolder()
	{
		// how to kown the folder is builded is a problem
		if(!loadMyFolder)  
			return;
		
		String list[] = getResources().getStringArray(R.array.myfolder);
		...
		for(int i = 0; i < count; i ++)
		{
			...
			if(id == -1L)
			{
				id = addMyFolderToData(list[i],i,3);
				if(id != -1L)
				{
					mFodlerHasBuild[i] = false;
				}
				store(mPreferences,KEY[i],id);

			}
			else 
			{
				mFodlerHasBuild[i] = true;
			}
		}
	}

	private Long addMyFolderToData(String name,int cellX,int cellY)
	{
		Log.d(TAG,"addMyFolderToData");
		CellLayout.CellInfo cellInfo = new CellLayout.CellInfo();
		cellInfo.cellX = cellX;
		cellInfo.cellY=cellY;
		cellInfo.screen=mWorkspace.getCurrentScreen();
		cellInfo.spanX=1;
		cellInfo.spanY=1;
		cellInfo.valid = true;
        ...
        // Create the view
	        FolderIcon newFolder = FolderIcon.fromXml(R.layout.folder_icon, this,
	                (ViewGroup) mWorkspace.getChildAt(mWorkspace.getCurrentScreen()), folderInfo);
	        mWorkspace.addInCurrentScreen(newFolder,
	                cellInfo.cellX, cellInfo.cellY, 1, 1, false);

		return folderInfo.id;
	}

	private void addApplicationToFolder(ArrayList<ApplicationInfo> apps)
	{
		if(apps == null)
			return ;
		...
		for(int i = 0; i < KEY.length; i ++)
		{
			if(!mFodlerHasBuild[i])
			{
				// this Folder was builded just before, read the folder id from file
				long id = read(mPreferences,KEY[i]);
				String packageName[] = null;
				
				if(id != -1L)
				{
					if(i == 0)  // posotoin 0 is Game in my Example
					{
						// define the package name of applications which want to add to Folder
						packageName = new String[] {"com.android.speechrecorder"};
					}
					else if(i == 1) // position 1 is Tool in my Example
					{
						packageName = new String[] {"com.cooliris.media","com.android.settings","com.android.browser"};
					}
					else if(i == 2) // position 1 is Video in my Example
					{
						packageName = new String[] {"android.rk.RockVideoPlayer","com.rk.youtube"};
					}

					addApplicationToFolder(apps,packageName,id);
				}
			}
		}
	}*/
	...
	/**
	* apps : the list of all application installed in box
	* packName:  package name of application which wanted show in Folder
	* id:    the Folder id
	*/
/*
	private void addApplicationToFolder(ArrayList<ApplicationInfo> apps,String[] packName,long id)
	{
		Log.d(TAG,"addApplicationToFolder, id = "+id);
		if((apps == null) || (packName == null))
			return ;

		for(int i = 0; i < apps.size(); i++)
		{
			ApplicationInfo application = apps.get(i);

			String name = application.title.toString();
//			Log.d(TAG,"application title = "+application.title.toString());
//			Log.d(TAG,"application intent = "+application.intent);
//			Log.d(TAG,"application container = "+application.container);

			ComponentName component = application.intent.getComponent();
			String packageName = component.getPackageName();
			String className = component.getClassName();

			for(int j = 0; j < packName.length; j ++)
			{
				if(packageName.equals(packName[j]))
				{
					ShortcutInfo item = application.makeShortcut();
					UserFolderInfo folderInfo =
	                                            LauncherModel.findOrMakeUserFolder(sFolders, id);

					Log.d(TAG,"folderInfo title = "+folderInfo.title.toString());
	                            folderInfo.add(item);
					
					LauncherModel.addOrMoveItemInDatabase(this,item,id,0,0,0);
				}
			}
		}
	...
	public void store(SharedPreferences preference,String name,long folderId)
	{
		if(preference != null)
		{
			SharedPreferences.Editor editor = mPreferences.edit();
			editor.putLong(name, folderId);
			editor.commit();
		}
	}

	public long read(SharedPreferences preference,String name)
	{
		if(preference != null)
		{
