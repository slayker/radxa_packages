...
//	 public void addMyFolder();
    ...
    private boolean filterPack(String packname){
		if(packname.startsWith("android.rk.RockVideoPlayer")){return true;}
		if(packname.startsWith("com.android.music")){return true;}
		if(packname.startsWith("com.android.gallery3d")){return true;}
		if(packname.startsWith("com.rk_itvui.allapp")){return true;}
		if(packname.startsWith("com.rk.setting")){return true;}
		if(packname.startsWith("com.android.settings")){return true;}
		return false;
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
	     ...
	     if(filterPack(packageName)){
		  if (DEBUG_LOADERS) Log.d(TAG, packageName + "is default app, NOT PERMISSION TO Remove or update!");
		  return ;
	     }
    ...
    private class LoaderTask implements Runnable {
        ...
        private boolean checkItemPlacement(ItemInfo occupied[][][], ItemInfo item) {
	     ...
	     if(item.screen>0){return false;}

            for (int x = item.cellX; x < (item.cellX+item.spanX); x++) {
                for (int y = item.cellY; y < (item.cellY+item.spanY); y++) {
		      Log.d(TAG, "item.screen = " + item.screen + ";item.cellX=" + x + "; item.cellY=" +y);
	...
	//		   callbacks.addMyFolder();
        ...
        private void loadAndBindAllApps() {
            ...
            if (!mAllAppsLoaded) {
                ...
                /*change language,appUI no change. by ywj 2011/1/20*/
                //mAllAppsLoaded = true;
            } else {
                onlyBindAllApps();
            }
    ...
    public ShortcutInfo getShortcutInfo(PackageManager manager, Intent intent, Context context,
	   ...
	   //icon = getDefaultAppIcon(context, componentName.getPackageName());
    ...
    private Bitmap getDefaultAppIcon(Context context, String pack){
		int resourceID = 0;
		if(true == pack.startsWith("android.rk.RockVideoPlayer")){
			resourceID = R.drawable.video;
		}
		if(true == pack.startsWith("com.android.music")){
			resourceID = R.drawable.music;
		}
		if(true == pack.startsWith("com.cooliris.media")){
			resourceID = R.drawable.picture;
		}
		if(true == pack.startsWith("com.rk_itvui.allapp")){
			resourceID = R.drawable.application;
		}
		if(true == pack.startsWith("com.rk.setting")){
