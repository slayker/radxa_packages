...
import com.rockchip.settings.inputmethod.TtsEnginePreference.RadioButtonGroupState;
...
public class TextToSpeechFragment extends SettingsPreferenceFragment implements
	...
	/**
	 * These look like birth years, but they aren't mine. I'm much younger than
	 * this.
	 */
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mTts = new TextToSpeech(getActivity().getApplicationContext(),
				mInitListener);
	...
	public void onPause() {
		super.onPause();
		if ((mDefaultRatePref != null)
				&& (mDefaultRatePref.getDialog() != null)) {
	...
	private void initSettings() {
		...
		if (getActivity() instanceof PreferenceActivity) {
			...
			throw new IllegalStateException(
					"TextToSpeechSettings used outside a "
							+ "PreferenceActivity");
		}
		...
		for (EngineInfo engine : engines) {
			TtsEnginePreference enginePref = new TtsEnginePreference(
					getActivity(), engine, this, preferenceActivity);
	...
	private void maybeUpdateTtsLanguage(String currentEngine) {
		if (currentEngine != null && mTts != null) {
			final String localeString = mEnginesHelper
					.getLocalePrefForEngine(currentEngine);
			if (localeString != null) {
				final String[] locale = TtsEngines
						.parseLocalePref(localeString);
				final Locale newLocale = new Locale(locale[0], locale[1],
						locale[2]);
				final Locale engineLocale = mTts.getLanguage();

				if (!newLocale.equals(engineLocale)) {
					if (DBG)
						Log.d(TAG, "Loading language ahead of sample check : "
								+ locale);
	...
	private void getSampleText() {
		...
		try {
			...
			Log.e(TAG, "Failed to get sample text, no activity found for "
					+ intent + ")");
		}
	}
	...
	private void onSampleTextReceived(int resultCode, Intent data) {
		...
		if (sample != null && mTts != null) {
			...
			Log.e(TAG,
					"Did not have a sample string for the requested language");
		}
	}
	...
	public boolean onPreferenceChange(Preference preference, Object objValue) {
		if (KEY_DEFAULT_RATE.equals(preference.getKey())) {
			...
			try {
				Settings.Secure.putInt(getContentResolver(), TTS_DEFAULT_RATE,
						mDefaultRate);
				if (mTts != null) {
					mTts.setSpeechRate(mDefaultRate / 100.0f);
				}
				if (DBG)
					Log.d(TAG, "TTS default rate changed, now " + mDefaultRate);
	...
	private void displayDataAlert(final String key) {
		...
		builder.setMessage(getActivity().getString(
				...
				mEnginesHelper.getEngineInfo(key).label));
	...
	private void updateDefaultEngine(String engine) {
		...
		if (DBG)
			Log.d(TAG, "Updating engine : Attempting to connect to engine: "
					+ engine);
		mTts = new TextToSpeech(getActivity().getApplicationContext(),
				mUpdateListener, engine);
	}

	/*
	 * Step 3: We have now bound to the TTS engine the user requested. We will
	 * attempt to check voice data for the engine if we successfully bound to
	 * it, or revert to the previous engine if we didn't.
	 */
	...
	private void checkVoiceData(String engine) {
		...
		try {
			if (DBG)
				Log.d(TAG,
						"Updating engine: Checking voice data: "
								+ intent.toUri(0));
			startActivityForResult(intent, VOICE_DATA_INTEGRITY_CHECK);
		} catch (ActivityNotFoundException ex) {
			Log.e(TAG, "Failed to check TTS data, no activity found for "
					+ intent + ")");
		}
	}
	...
	private void onVoiceDataIntegrityCheckDone(Intent data) {
		...
		Settings.Secure.putString(getContentResolver(), TTS_DEFAULT_SYNTH,
				engine);
