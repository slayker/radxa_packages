 ...
 * 
 * CredentialStorage has a pretty convoluted state machine to migrate from the
 * old style separate keystore password to a new key guard based password, as
 * well as to deal with setting up the key guard if necessary.
 * 
 * KeyStore: UNINITALIZED KeyGuard: OFF Action: set up key guard Notes: factory
 * state
 * 
 * KeyStore: UNINITALIZED KeyGuard: ON Action: confirm key guard Notes: user had
 * key guard but no keystore and upgraded from pre-ICS OR user had key guard and
 * pre-ICS keystore password which was then reset
 * 
 * KeyStore: LOCKED KeyGuard: OFF/ON Action: old unlock dialog Notes: assume old
 * password, need to use it to unlock. if unlock, ensure key guard before
 * install. if reset, treat as UNINITALIZED/OFF
 * 
 * KeyStore: UNLOCKED KeyGuard: OFF Action: set up key guard Notes: ensure key
 * guard, then proceed
 * 
 * KeyStore: UNLOCKED keyguard: ON Action: normal unlock/install Notes: this is
 * the common case
 */
public final class CredentialStorage extends Activity {
	...
	/**
	 * After unsuccessful KeyStore.unlock, the number of unlock attempts
	 * remaining before the KeyStore will reset itself.
	 * 
	...
	/**
	 * Based on the current state of the KeyStore and key guard, try to make
	 * progress on unlocking or installing to the keystore.
	 */
	...
	/**
	 * Prompt for reset confirmation, resetting on confirmation, finishing
	 * otherwise.
	 */
	private class ResetDialog implements DialogInterface.OnClickListener,
	 ...
	 */
	private class ResetKeyStoreAndKeyChain extends
		...
		protected Boolean doInBackground(Void... unused) {
			...
			try {
				KeyChainConnection keyChainConnection = KeyChain
						.bind(CredentialStorage.this);
				try {
		...
		protected void onPostExecute(Boolean success) {
			if (success) {
				...
				msg.what = SettingMacroDefine.upCredentialStatusSetting;
				mHandler.sendMessage(msg);
				
			} else {
				Toast.makeText(CredentialStorage.this,
						R.string.credentials_not_erased, Toast.LENGTH_SHORT)
						.show();
			}
			finish();
		}
	 ...
	 */
	private class ConfigureKeyGuardDialog implements
		...
		public void onDismiss(DialogInterface dialog) {
			if (mConfigureConfirmed) {
				...
				intent.putExtra("minimum_quality", MIN_PASSWORD_QUALITY);
	 ...
	 * 
	 * On successful unlock, ensure migration to key guard before continuing. On
	 * unsuccessful unlock, retry by calling handleUnlockOrInstall.
	 */
	private class UnlockDialog implements TextWatcher,
		...
		private UnlockDialog() {
			View view = View.inflate(CredentialStorage.this,
			...
			if (mRetriesRemaining == -1) {
				...
				text = getResources().getText(
						R.string.credentials_wrong_password);
			} else if (mRetriesRemaining == 1) {
				text = getResources().getText(
						R.string.credentials_reset_warning);
			} else {
				text = getString(R.string.credentials_reset_warning_plural,
						mRetriesRemaining);
			}
			...
			/*		.setBackground(R.drawable.tv_popup_full_dark,R.drawable.tv_popup_top_dark, R.drawable.tv_popup_center_dark,
			   				R.drawable.tv_popup_bottom_dark,R.drawable.tv_popup_full_bright,R.drawable.tv_popup_top_bright,
			   				R.drawable.tv_popup_center_bright,R.drawable.tv_popup_bottom_bright,R.drawable.tv_popup_bottom_medium)*/
					.setView(view).setTitle(R.string.credentials_unlock)
					.setPositiveButton(android.R.string.ok, this)
					.setNegativeButton(android.R.string.cancel, this).create();
			dialog.setOnDismissListener(this);
			dialog.show();
		...
		public void afterTextChanged(Editable editable) {
			mButton.setEnabled(mOldPassword == null
					|| mOldPassword.getText().length() > 0);
		}
		...
		public void onDismiss(DialogInterface dialog) {
			if (mUnlockConfirmed) {
				...
				if (error == KeyStore.NO_ERROR) {
					mRetriesRemaining = -1;
					Toast.makeText(CredentialStorage.this,
							R.string.credentials_enabled, Toast.LENGTH_SHORT)
							.show();
					...
					mRetriesRemaining = -1;
					Toast.makeText(CredentialStorage.this,
							R.string.credentials_erased, Toast.LENGTH_SHORT)
							.show();
